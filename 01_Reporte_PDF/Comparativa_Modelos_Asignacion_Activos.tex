...
\noindent Se evalúa el desempeño fuera de muestra de siete modelos de asignación de activos sobre un universo de nueve ETFs líquidos entre 2007 y 2026. La comparación abarca la cartera de referencia 60/40, el modelo equiponderado (1/N), carteras clásicas de media-varianza (Markowitz MVO y Markowitz Remuestreado por Bootstrap), modelos de riesgo de cola (Mínimo CVaR y Mínimo CDaR) y el algoritmo de Asignación Jerárquica de Contribución de Riesgo Equitativo (HERC). El marco de análisis incorpora un protocolo \textit{Walk-Forward} con deriva diaria de precios, costos de transacción de 5 puntos básicos, cuantificación de pérdidas extremas mediante Teoría de Valores Extremos (Pareto Generalizada), Validación Cruzada Combinatoria con Embargo (CPCV), y contrastación de significancia con Probabilistic Sharpe Ratio (PSR) y Deflated Sharpe Ratio (DSR). Los resultados muestran que ningún modelo domina al resto en todas las métricas: la cartera 60/40 registra el mayor CAGR neto (9.64\%) y el mejor Sharpe (0.78), Markowitz MVO el mayor retorno acumulado absoluto (495.64\%), Min-CVaR la menor pérdida en eventos de cola, y HERC la mayor diversificación efectiva ($N_{\text{eff}} = 6.46$) con la rotación más baja de los modelos cuantitativos (31.43\%).\\[0.35cm]
...
\subsection{Extensión: Optimización Distribucionalmente Robusta (DRO) mediante la Métrica de Wasserstein}
\label{sec:dro}
...
\end{equation}
El único término nuevo respecto del CVaR empírico (ecuación 7) es la penalización $\frac{\varepsilon}{1-\alpha}\|w\|_2$ --un cono de segundo orden, no una desigualdad lineal, por lo que esta extensión se resuelve con un solver SOCP (CLARABEL) en vez de programación lineal pura. Con $\varepsilon = 0$ el problema colapsa exactamente al Mínimo CVaR original (verificado numéricamente: diferencia máxima entre soluciones $< 10^{-8}$). El radio se fija en $\varepsilon = 0.02$, calibrado como el orden de magnitud del error de estimación muestral típico de $\hat{\mu}$ sobre una ventana de 756 ruedas para este universo.
...
\subsection{Extensión: Implementación del Marco Generativo Ex-Ante}
\label{sec:exante}

Esta sección implementa literalmente el marco que la Subsección 4.1 describe pero no ejecuta. El procedimiento opera activo por activo (no a nivel portafolio, a diferencia del filtrado ex-post de la ecuación 12):
...
\section{Extensión Metodológica: Robustez Distribucional y Marco Generativo Ex-Ante}
\label{sec:extension}

Esta sección reporta los resultados fuera de muestra de las dos extensiones descritas en las Subsecciones \ref{sec:dro} y \ref{sec:exante}, evaluadas bajo el \textbf{mismo} protocolo Walk-Forward, universo de 9 activos, costos de transacción (5pb) y período (2010--2026) que el resto del documento -- ningún parámetro experimental cambia, solo el modelo de asignación.

\begin{table}[H]
\centering
\renewcommand{\arraystretch}{1.18}
\caption{Extensión: DRO-Wasserstein y Marco Generativo Ex-Ante, contra los dos modelos de referencia más cercanos de la Tabla~\ref{tab:metricas} (2010--2026).}
