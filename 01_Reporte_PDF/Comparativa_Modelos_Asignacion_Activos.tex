...
Esta es la forma \textit{dual} del problema, expresada como maximización de utilidad. Markowitz (1952) lo planteó originalmente en su forma \textit{primal}: minimizar la varianza de la cartera sujeta a un retorno mínimo objetivo $r^*$:
\begin{equation*}
\min_{w} \quad w^T \Sigma w \quad \text{sujeto a} \quad \mu^T w \ge r^*, \quad \mathbf{1}^T w = 1, \quad w_i \ge 0 \quad \forall i
\end{equation*}
Ambas formulaciones son matemáticamente equivalentes: variar $\gamma$ en la ecuación (1) traza la misma frontera eficiente que variar $r^*$ en el problema primal, y cada punto de la frontera resuelve una versión de ambos problemas para algún par $(\gamma, r^*)$ correspondiente. La forma con aversión al riesgo es la más citada en la práctica de gestión activa de carteras (Grinold y Kahn, 2000) y es la que se implementa en este trabajo. La forma con restricción de retorno es la más fiel al artículo original de 1952 y la que suele aparecer en los cursos introductorios de teoría de carteras.

Estimar $\mu$ y $\Sigma$ directamente de la muestra tiene limitaciones prácticas serias. Michaud (1989) demostró que el optimizador de media-varianza actúa como amplificador de errores muestrales. Los activos con rendimientos sobrestimados o varianzas subestimadas reciben ponderaciones desproporcionadas. Por eso las carteras óptimas tienden a concentrarse en pocos instrumentos y muestran alta inestabilidad temporal, lo que induce rotaciones excesivas.
...
Los resultados de los contrastes son:
\begin{enumerate}
    ...
    \item \textbf{Heterocedasticidad condicional y efectos ARCH.} El estadístico $Q(10)$ de Ljung-Box sobre rendimientos al cuadrado ($r_t^2$) da $p < 0.0001$ en todos los activos. Eso confirma agrupamiento de volatilidad y descarta el supuesto de varianza constante en retornos brutos (Engle, 1982).
...
\caption{Indicadores de desempeño, riesgo, colas extremas y operatividad fuera de muestra (2010--2026) para los nueve modelos evaluados.}
\label{tab:metricas}
\resizebox{\textwidth}{!}{%
...
Sus resultados dependen del régimen. El ciclo 2010--2026 estuvo dominado por expansión tecnológica y tasas reales negativas. En la práctica es casi un portafolio de dos activos ($N_{\text{eff}} = 1.92$). En 2022, la correlación positiva entre bonos y acciones produjo una caída del $-21.18\%$. Eso pone de manifiesto que la diversificación entre clases no siempre opera en episodios de contracción simultánea.

\subsection{Markowitz Media-Varianza}

Markowitz MVO registra el mayor retorno acumulado del universo (CAGR 11.39\%, acumulado 495.64\%), pero con el peor perfil de riesgo. La volatilidad anual es de 17.05\%, el retroceso máximo de $-39.32\%$, el $\text{CVaR}_{99\%}^{\text{EVT}}$ de $-3.56\%$ y la rotación anual de 288.75\%. El Sharpe de 0.55 es el más bajo. El retorno adicional frente al 60/40 no compensa el riesgo en ninguna métrica ajustada. La concentración en $N_{\text{eff}} = 1.31$ activos es consecuencia del comportamiento de amplificador de errores documentado por Michaud (1989): errores pequeños en $\hat{\mu}$ se amplifican en las ponderaciones y concentran la cartera en activos con alto retorno histórico reciente.
...
\end{equation}
Con Sharpe de 0.55 y $\sigma = 17.05\%$, Markowitz genera retrocesos esperados considerablemente mayores que HERC (Sharpe 0.65, $\sigma = 7.35\%$) en cualquier horizonte.

Grossman y Zhou (1993) demostraron que bajo un mandato de preservación de capital con límite de retroceso, la solución óptima no es la cartera de media-varianza, sino una que gestione activamente la distancia al máximo histórico. Este resultado justifica formalmente los modelos Min-CDaR y, en menor medida, HERC como alternativas a Markowitz cuando el mandato incluye restricciones de pérdida.
...
Markowitz MVO acumula el mayor retorno fuera de muestra (495.64\%), por encima del 60/40 (358.51\%), DRO-Wasserstein (228.66\%) y HERC (197.22\%). Ese retorno se explica por la concentración en renta variable tecnológica durante un ciclo favorable para esa clase de activos. Se obtiene con el mayor retroceso del universo ($-39.32\%$) y el Sharpe más bajo (0.55): el riesgo adicional asumido no fue compensado proporcionalmente.

Markowitz MVO rota a 288.75\% anual y Michaud a 503.76\%. HERC opera a 31.43\% y DRO-Wasserstein a 3.77\%, la rotación más baja de las nueve estrategias. Mínimo CVaR Ex-Ante es la excepción que confirma que la rotación no depende solo de la robustez matemática del modelo. Con 413.00\% anual, su reasignación mensual es tan agresiva como la de Michaud, pese a partir de un marco de calibración de colas más sofisticado que el resto del documento. Markowitz concentra el capital en $N_{\text{eff}} = 1.31$ activos. HERC alcanza $N_{\text{eff}} = 6.46$, con $\text{PSR} = 99.58\%$, $\text{DSR} = 98.49\%$ y $\text{CVaR}_{99\%}^{\text{EVT}} = -1.53\%$ frente a $-3.56\%$ de Markowitz. DRO-Wasserstein llega a $N_{\text{eff}} = 8.92$, la diversificación efectiva más alta entre los modelos que estiman parámetros. Solo 1/N, que no estima nada, la supera.
...
\section{Apalancamiento y Réplica vía Opciones sin Acceso a Margen Institucional}
\label{sec:argentina}

Los nueve modelos anteriores se evalúan sin apalancamiento ($\sum w_i = 1$, $w_i \ge 0$): esa restricción fija la asignación \textit{relativa} entre activos, pero deja abierta una decisión ortogonal, cuánta exposición bruta tomar por unidad de capital propio, que esta sección resuelve para dos de las carteras del estudio, la referencia 60/40 y HERC. Se contrastan dos mecanismos de acceso a esa exposición adicional: margen institucional a la tasa corta de EE.\,UU.\ (el supuesto habitual de la literatura de risk parity apalancado, Bridgewater/AQR) frente a opciones listadas, la vía disponible para un inversor sin ese acceso, el caso típico de una cartera minorista, con o sin residencia argentina.

\subsection{Apalancamiento vía margen: proyección analítica sobre los resultados publicados}

Se aplica el marco de \textit{volatility targeting} (Ecuación de \textit{leverage} estándar en risk parity institucional): dado un objetivo de volatilidad anual $\sigma^*$, el multiplicador $L = \sigma^* / \sigma_{\text{cartera}}$, acotado a un máximo $L_{\max} = 2.0$ (límite típico de margen de bróker). Con financiamiento a tasa $r_f$ sobre el capital prestado, el retorno aritmético y el CAGR resultantes se aproximan, a primer y segundo orden (arrastre por volatilidad, $g \approx r - \sigma^2/2$), como:
\begin{equation}
r_L \approx L \, r_u - (L-1) \, r_f, \qquad \text{CAGR}_L \approx r_L - \frac{(L\,\sigma_u)^2}{2}, \qquad \sigma_L = L\,\sigma_u
\end{equation}
donde $r_u \approx \text{CAGR}_u + \sigma_u^2/2$ se obtiene deshaciendo la misma aproximación sobre el CAGR ya publicado en la Tabla~\ref{tab:metricas}. Con $\sigma^* = 15\%$ y $r_f = 4.30\%$ (T-Bill de 13 semanas más 50\,pb de spread de bróker, promedio reciente):

\begin{table}[H]
\centering
\caption{Proyección analítica de apalancamiento vía margen sobre 60/40 y HERC ($\sigma^*=15\%$, $r_f=4.30\%$). No es una nueva corrida walk-forward.}
\label{tab:leverage-paper}
\resizebox{\textwidth}{!}{%
\begin{tabular}{lrrrr}
\toprule
\textbf{Cartera} & \textbf{$L$} & \textbf{CAGR proyectado} & \textbf{Vol. proyectada} & \textbf{Sharpe proyectado} \\
\midrule
60/40 (base: CAGR 9.64\%, $\sigma$ 9.79\%, Sharpe 0.781) & 1.53$\times$ & 12.09\% & 14.98\% & 0.807 \\
HERC (base: CAGR 6.81\%, $\sigma$ 7.35\%, Sharpe 0.654) & 2.00$\times$\footnotemark & 8.78\% & 14.70\% & 0.597 \\
\bottomrule
\end{tabular}%
}
\end{table}
\footnotetext{HERC satura en el tope $L_{\max}=2.0$: $\sigma^*/\sigma_u = 15\%/7.35\% = 2.04 > 2.0$. El objetivo de volatilidad no es la restricción activa en este caso. El tope de margen lo es.}

La lectura directa sugeriría que 60/40 mejora levemente su Sharpe apalancado y HERC lo empeora. \textbf{Esta proyección debe leerse como una cota optimista, no como una predicción confiable.} Un backtest walk-forward independiente de HERC sobre el universo ampliado de 26 activos del motor principal (2003--2026), simulando el mecanismo de apalancamiento con margin calls e interés compuesto día a día en vez de la aproximación de primer/segundo orden de esta subsección, y con el mismo objetivo de volatilidad (15\%), da un Sharpe realizado de 0.621, sensiblemente por debajo de la proyección analítica de la Tabla~\ref{tab:leverage-paper}. La diferencia es el costo de financiamiento compuesto sobre un capital prestado que crece con cada mala racha y la fricción de volatilidad de una cartera de mínima varianza forzada a un objetivo de riesgo muy por encima de su volatilidad natural. Ningún término de esta aproximación captura la dependencia del camino ni el riesgo de \textit{margin call}. Los números de esta subsección deben tratarse como un límite superior optimista del desempeño apalancado, no como su estimación puntual.

\subsection{Apalancamiento y cobertura vía opciones: réplica accesible sin margen}

Para un inversor sin línea de margen institucional, el caso de la enorme mayoría de cuentas minoristas y estructuralmente el de un inversor argentino sin acceso a tasas de fondeo en dólares, la alternativa es replicar el efecto de apalancamiento o cobertura con opciones listadas sobre los mismos nueve ETFs de este estudio, que cotizan con cadenas de opciones líquidas en CBOE. Se precian con Black-Scholes-Merton (Black \& Scholes, 1973; Merton, 1973) usando la volatilidad realizada de cada activo en los últimos 3 años (no volatilidad implícita, que requeriría una cadena de opciones en vivo) y la misma tasa de financiamiento de la Subsección 9.1.

\begin{table}[H]
\centering
\caption{Costo de tres estrategias de opciones a 3 meses (put protector 90\%, call vendido 110\%, collar neto) y de una LEAPS sintética a 1 año (call 80\%), sobre volatilidad realizada real de 3 años. Todos los valores en \% del nocional.}
\label{tab:options-paper}
\resizebox{\textwidth}{!}{%
\begin{tabular}{lrrrrrr}
\toprule
\textbf{Ticker} & \textbf{Vol. 3a} & \textbf{Put 90\% (3m)} & \textbf{Call 110\% (3m)} & \textbf{Collar neto (3m)} & \textbf{LEAPS 80\% (1a)} & \textbf{Exp. embebida} \\
\midrule
SPY & 15.30\% & 0.203\% & 0.542\% & $-0.339\%$ (crédito) & 23.59\% & $\sim$4.1$\times$ \\
QQQ & 20.35\% & 0.606\% & 1.206\% & $-0.600\%$ (crédito) & 24.16\% & $\sim$3.8$\times$ \\
EEM & 19.52\% & 0.527\% & 1.086\% & $-0.559\%$ (crédito) & 24.04\% & $\sim$3.9$\times$ \\
VNQ & 16.91\% & 0.310\% & 0.735\% & $-0.425\%$ (crédito) & 23.73\% & $\sim$4.0$\times$ \\
TLT & 13.75\% & 0.122\% & 0.378\% & $-0.256\%$ (crédito) & 23.49\% & $\sim$4.2$\times$ \\
IEF & 6.46\% & $\approx$0.000\% & 0.005\% & $-0.005\%$ (crédito) & 23.37\% & $\sim$4.3$\times$ \\
LQD & 7.25\% & 0.001\% & 0.013\% & $-0.013\%$ (crédito) & 23.37\% & $\sim$4.3$\times$ \\
GLD & 20.70\% & 0.642\% & 1.259\% & $-0.617\%$ (crédito) & 24.22\% & $\sim$3.8$\times$ \\
\bottomrule
\end{tabular}%
}
\end{table}
{\footnotesize DBC (materias primas) se excluye de esta tabla: no está disponible en el conjunto de datos del motor principal con el que se calculó, y este documento no fabrica una volatilidad de reemplazo para un activo sin datos reales.}

\begin{figure}[H]
\centering
\includegraphics[width=0.98\textwidth]{fig08_costo_opciones_apalancamiento_argentina.png}
\caption{Costo del collar 90/110 a 3 meses (izquierda) y de la LEAPS 80\% a 1 año frente al costo de margen (derecha), por ticker.}
\label{fig:opciones-argentina}
\end{figure}

Dos hallazgos directos:

\begin{enumerate}
\item \textbf{El collar 90/110 es, en los ocho activos, un crédito neto}: el call vendido financia por completo, y con margen, al put comprado. Es la estrategia de cobertura de menor fricción de todo este documento: protección de cola sin costo de prima neto, ejecutable sin ninguna línea de margen. El costo real no es la prima (negativa) sino el upside resignado por encima del strike del call vendido, no capturado en esta tabla de primas puntuales.
\item \textbf{La LEAPS sintética es estructuralmente cara.} Comprar exposición apalancada de un año vía una call profunda (80\% moneyness) cuesta 23--24\% del nocional en los ocho activos, un orden de magnitud por encima del costo de financiamiento de margen (4.30\% anual) de la Subsección 9.1. Es la prima que se paga por no tener acceso a margen institucional, no una forma barata de replicarlo: el mismo backtest walk-forward de la Subsección 9.1 muestra que, sobre HERC, esta vía produce una volatilidad del camino y un drawdown máximo dramáticamente peores que el margen, con un CAGR de largo plazo similar.
\end{enumerate}

\section{Auditoría de Robustez: Broken Pipe Test (Ruido Blanco al 50\% del Universo)}
\label{sec:brokenpipe}

Los nueve modelos de este documento se validan fuera de muestra, pero eso por sí solo no
descarta una fuga de información dentro del pipeline (\textit{look-ahead bias}, fuga entre
particiones de CPCV, o un error de implementación que fabrique retorno donde no lo hay). La
prueba de control estándar para esa clase de error es reemplazar una fracción del universo
por ruido blanco puro y verificar que el motor real no le atribuye retorno significativo:
ningún activo i.i.d.\ de media cero puede tener contribución esperada distinta de cero a
una cartera optimizada correctamente.

\textbf{Metodología.} Se reemplazan 4 de los 9 tickers (44\%, la fracción entera más
cercana al 50\% con 9 activos) por ruido blanco gaussiano en cada corrida, y se ejecuta el
motor HERC walk-forward real de este documento (misma lógica de costo y deriva de pesos que
la Sección 5.1) más CPCV (Sección 5.2) y el Deflated Sharpe Ratio (Sección 5.3), sobre 40
semillas independientes. La corrida es sobre \textbf{mercado real}
(\texttt{precios\_ajustados.csv}, los mismos nueve activos y período 2007--2026 de todo este
documento), no un universo sintético.

\textbf{Hallazgo metodológico.} La lectura literal del ejercicio --calibrar el ruido a la
misma media del activo real que reemplaza-- le da al ruido, sobre un universo con prima de
riesgo positiva real (SPY/QQQ 2007--2026), la misma esperanza que el activo real: una
estrategia que cobra esa prima no está filtrando información, está cobrando correctamente
lo que el propio generador de ruido le regaló. La premisa lógica del test (``ningún activo
de ruido i.i.d.\ puede tener retorno esperado positivo'') sólo vale si el ruido es de media
cero. Se corrigió a \texttt{noise\_mean\_mode=`zero'} (media exactamente 0) como
configuración correcta, y se corrió también \texttt{`matched'} para exhibir el falso
positivo esperado:

\begin{table}[H]
\centering
\caption{Broken Pipe Test sobre mercado real (2007--2026): contribución diaria del sleeve de ruido, 40 semillas independientes por modo.}
\label{tab:brokenpipe-modes}
\begin{tabular}{lrrl}
\toprule
\textbf{Modo} & \textbf{Contribución media diaria} & \textbf{$p$-valor} & \textbf{Veredicto} \\
\midrule
\texttt{zero} (correcto) & $+0.000008$ & 0.176 & Sin evidencia de fuga \\
\texttt{matched} (lectura literal) & $+0.000100$ & $<0.0001$ & Fuga clara (artefacto esperado) \\
\bottomrule
\end{tabular}
\end{table}

Con 20 semillas el $p$-valor del modo \texttt{zero} había cruzado el umbral 0.05 por poco
(0.026); se subió a 40 semillas antes de reportar un veredicto, mismo criterio de potencia
estadística del resto de este documento (Sección 5.3) en vez de concluir sobre un
$p$-valor marginal con pocos ensayos.

\begin{table}[H]
\centering
\caption{Métricas agregadas del Broken Pipe Test, modo \texttt{zero} (40 semillas).}
\label{tab:brokenpipe-metrics}
\begin{tabular}{lr}
\toprule
\textbf{Métrica} & \textbf{Valor} \\
\midrule
Sharpe medio del portafolio (entre semillas) & 0.443 \\
Contribución media diaria del sleeve de ruido & $+0.000008$ \\
Contribución, $p$-valor (vs.\ 0) & 0.176 \\
Deflated Sharpe Ratio & 0.221 \\
Semillas independientes & 40 \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Veredicto: sin evidencia de fuga de información} en el motor HERC walk-forward de
este documento. Reproducible con:
\begin{quote}
\texttt{python 03\_Codigo/broken\_pipe\_test.py}
\end{quote}

\subsection{Nota complementaria: distancia de correlación absoluta para coberturas}

La fórmula clásica de distancia angular de la Sección 2 ($d = \sqrt{0.5(1-\rho)}$) asigna a
una cobertura con $\rho=-0.9$ una distancia $d=0.975$ --más lejos que un activo sin ninguna
relación ($\rho=0 \Rightarrow d=0.707$)-- porque trata la correlación negativa fuerte como
disimilitud, no como la relación de cobertura que en realidad representa. Una distancia
alternativa $D = 1-|\rho|$ coloca a esa misma cobertura a $D=0.1$, en el vecindario
inmediato del árbol jerárquico. En el universo de nueve activos de este documento las
coberturas no llegan a ese extremo --TLT/IEF frente a la renta variable rondan
$\rho \approx -0.3$ a $-0.4$ en 2007--2026 (Tabla~\ref{tab:diagnostico}), un caso más
moderado que el ejemplo de $\rho=-0.9$-- pero el mismo argumento matemático aplica en
magnitud proporcional. La función queda disponible (\texttt{calcular\_distancia\_angular\_abs\_corr}
en \texttt{03\_Codigo/broken\_pipe\_test.py}) sin cablearse dentro de
\texttt{pipeline\_optimizacion.py}: cambiar la distancia por defecto regeneraría todas las
figuras y tablas ya publicadas en este documento sin que se haya pedido explícitamente ese
...
Grinold, R. C., \& Kahn, R. N. (2000). \textit{Active Portfolio Management: A Quantitative Approach for Producing Superior Returns and Controlling Risk} (2nd ed.). McGraw-Hill.
...
Merton, R. C. (1973). Theory of rational option pricing. \textit{The Bell Journal of Economics and Management Science}, 4(1), 141--183.
