...
\caption{Indicadores de desempeño, riesgo, colas extremas y operatividad fuera de muestra (2010--2026): 1/N, 60/40, Markowitz, Michaud, Mín. CVaR, Mín. CDaR y HERC. Véase la Tabla~\ref{tab:metricas_extension} para DRO-Wasserstein y el Marco Generativo Ex-Ante.}
\label{tab:metricas}
\resizebox{\textwidth}{!}{%
...
La Tabla~\ref{tab:metricas_extension} añade a la comparación anterior los dos modelos que incorporan explícitamente robustez distribucional (DRO-Wasserstein) y generación de escenarios sintéticos (Marco Generativo Ex-Ante), evaluados bajo el \textbf{mismo} protocolo Walk-Forward, universo de 9 activos, costos de transacción (5pb) y período (2010--2026) que el resto del documento -- ningún parámetro experimental cambia, solo el modelo de asignación.

\begin{table}[H]
\centering
\renewcommand{\arraystretch}{1.18}
\caption{Indicadores de desempeño para DRO-Wasserstein y Marco Generativo Ex-Ante, contra los dos modelos de referencia más cercanos de la Tabla~\ref{tab:metricas} (2010--2026).}
\label{tab:metricas_extension}
\resizebox{\textwidth}{!}{%
\begin{tabular}{lrrrr}
\toprule
\textbf{Indicador / Métrica} & \textbf{DRO-Wasserstein} & \textbf{Mín. CVaR Ex-Ante} & \textbf{Mín. CVaR (ref.)} & \textbf{HERC (ref.)} \\
\midrule
\multicolumn{5}{l}{\textit{\textbf{A. Rendimiento y Retorno Acumulado}}} \\
CAGR (\%) & 7.46 & 4.58 & 5.11 & 6.81 \\
Retorno Acumulado (\%) & 228.7 & 109.8 & 127.9 & 197.2 \\
\midrule
\multicolumn{5}{l}{\textit{\textbf{B. Riesgo y Desempeño Ajustado}}} \\
Volatilidad Anual (\%) & 8.92 & 5.32 & 5.32 & 7.35 \\
Desviación a la Baja (\%) & 9.52 & 5.55 & 5.54 & 7.82 \\
Ratio de Sharpe & 0.612 & 0.485 & 0.584 & 0.654 \\
Ratio de Sortino & 0.573 & 0.465 & 0.561 & 0.615 \\
Ratio de Calmar & 0.356 & 0.258 & 0.314 & 0.323 \\
Ratio de Omega & 1.120 & 1.091 & 1.108 & 1.126 \\
Ratio Ganancia/Pérdida & 1.165 & 1.165 & 1.182 & 1.182 \\
\midrule
\multicolumn{5}{l}{\textit{\textbf{C. Riesgo Extremo y Colas EVT}}} \\
Retroceso Máximo (\%) & -20.93 & -17.78 & -16.29 & -21.11 \\
CDaR 95\% (\%) & -14.90 & -12.78 & -12.77 & -15.35 \\
VaR 95\% Diario (\%) & -0.86 & -0.52 & -0.52 & -0.72 \\
CVaR 95\% Diario (\%) & -1.31 & -0.79 & -0.79 & -1.09 \\
VaR 99\% EVT (\%) & -1.44 & -0.83 & -0.83 & -1.17 \\
CVaR 99\% EVT (\%) & -1.84 & -1.06 & -1.05 & -1.53 \\
Coeficiente de Asimetría & -0.559 & -0.218 & -0.173 & -0.368 \\
Curtosis de Exceso & 9.16 & 4.52 & 3.69 & 7.75 \\
\midrule
\multicolumn{5}{l}{\textit{\textbf{D. Significancia y Operatividad}}} \\
PSR (vs 0) (\%) & 99.30 & 97.53 & 99.09 & 99.58 \\
DSR (\%) & 98.48 & 96.64 & 97.24 & 98.49 \\
Rotación Anualizada (\%) & 3.77 & 413.00 & 66.37 & 31.43 \\
Nº Efectivo de Activos ($N_{\text{eff}}$) & 8.92 & 1.93 & 1.84 & 6.46 \\
\bottomrule
\end{tabular}%
}
\end{table}

La Figura~\ref{fig:pesos_extension} muestra la contrapartida dinámica de la Tabla~\ref{tab:metricas_extension}: la evolución de la asignación de capital de ambos modelos a lo largo de los rebalanceos mensuales fuera de muestra, en el mismo formato que la Figura~\ref{fig:pesos} usa para el resto de los modelos. El contraste visual entre los dos paneles es directo: DRO-Wasserstein (Panel A) mantiene una estructura casi estática, mientras que Mínimo CVaR Ex-Ante (Panel B) exhibe reasignaciones bruscas y recurrentes --la contrapartida gráfica de la rotación de $413.00\%$ reportada en la Tabla~\ref{tab:metricas_extension}.

\begin{figure}[H]
\centering
\includegraphics[width=0.96\textwidth]{fig08_evolucion_pesos_extension.png}
\caption{Evolución temporal de la asignación de activos de DRO-Wasserstein (Panel A) y Mínimo CVaR Ex-Ante (Panel B) a lo largo de los rebalanceos mensuales fuera de muestra (2010--2026). Mismo protocolo Walk-Forward que la Figura~\ref{fig:pesos}.}
\label{fig:pesos_extension}
\end{figure}
...
\centering
\renewcommand{\arraystretch}{1.15}
\caption{Posición relativa vs. cartera 60/40. \checkmark: supera; $\circ$: comparable; $\times$: inferior.}
\label{tab:benchmark}
\begin{tabular}{lrrrr}
\toprule
\textbf{Métrica} & \textbf{Markowitz MVO} & \textbf{Mín. CVaR} & \textbf{HERC} & \textbf{1/N} \\
\midrule
...
$N_{\text{eff}}$ & $\times$ & $\times$ & \checkmark & \checkmark \\
DSR (\%) & $\times$ & $\times$ & $\circ$ & $\times$ \\
\bottomrule
\end{tabular}
\end{table}

Ningún modelo supera al 60/40 en Sharpe dentro del período analizado. Los modelos de colas y HERC tienen sentido cuando el mandato impone restricciones de pérdida o requiere mayor número de activos efectivos, no como alternativa de mayor retorno.
...
Durante las crisis de 2011, 2020 y 2022, HERC registró un retroceso máximo de $-21.11\%$, frente a $-39.32\%$ en Markowitz MVO y $-28.48\%$ en Michaud. En 2022 la correlación positiva entre bonos y acciones afectó al 60/40 ($-21.18\%$), mientras que la ponderación de HERC en materias primas (DBC) y oro (GLD) redujo la caída relativa.

Markowitz MVO acumula el mayor retorno fuera de muestra (495.64\%), por encima del 60/40 (358.51\%) y de HERC (197.22\%). Este resultado se explica por la concentración en renta variable tecnológica durante un ciclo favorable para esa clase de activos. Ese retorno se obtiene con el mayor retroceso del universo ($-39.32\%$) y el Sharpe más bajo (0.55): el riesgo adicional asumido no fue compensado proporcionalmente.

Markowitz MVO rota a 288.75\% anual y Michaud a 503.76\%; HERC opera a 31.43\%. Markowitz concentra el capital en $N_{\text{eff}} = 1.31$ activos. HERC alcanza $N_{\text{eff}} = 6.46$, con $\text{PSR} = 99.58\%$, $\text{DSR} = 98.49\%$ y $\text{CVaR}_{99\%}^{\text{EVT}} = -1.53\%$ frente a $-3.56\%$ de Markowitz.

De los dos modelos restantes, DRO-Wasserstein es el que mejor complementa a HERC dentro de este universo: rotación de apenas 3.77\% (la más baja del estudio) con un Sharpe (0.612) competitivo frente a Mínimo CVaR. El Marco Generativo Ex-Ante, en cambio, no traslada su rigor de calibración de colas a una mejora de desempeño cuando se lo usa como insumo directo del optimizador: su Sharpe (0.485) y su rotación (413.00\%) quedan peor posicionados que el propio Mínimo CVaR histórico que busca reemplazar.
