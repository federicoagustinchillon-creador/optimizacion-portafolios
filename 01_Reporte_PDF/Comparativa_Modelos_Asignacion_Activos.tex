...
Durante la crisis de deuda europea (2011), el colapso por COVID-19 (2020) y el episodio estanflacionario de 2022, HERC contuvo el retroceso máximo en $-21.11\%$, frente a $-39.32\%$ en Markowitz MVO y $-28.48\%$ en Michaud. En 2022, la correlación positiva entre bonos y acciones provocó una caída del $-21.18\%$ en la cartera 60/40, mientras que HERC mantuvo estabilidad gracias a la ponderación asignada a materias primas (DBC) y oro (GLD).

Respecto a la fricción operativa, Markowitz MVO registra una rotación anual del $288.75\%$ y Michaud del $503.76\%$, mientras que HERC opera con solo $31.43\%$, conteniendo el impacto por costos de transacción.

Markowitz concentra la cartera en $N_{\text{eff}} = 1.31$ activos efectivos. HERC distribuye el capital en $N_{\text{eff}} = 6.46$ activos efectivos, equilibrando la contribución marginal de riesgo entre grupos (Figura~\ref{fig:riesgo}). El ratio de Sharpe de HERC registra significancia estadística ($\text{PSR} = 99.58\%$, $\text{DSR} = 98.49\%$), con un $\text{CVaR}_{99\%}^{\text{EVT}}$ diario del $-1.53\%$ frente al $-3.56\%$ de Markowitz.
