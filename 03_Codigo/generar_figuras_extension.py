...
Salidas en 04_Figuras/:
  ...
  - fig08_evolucion_pesos_extension.png     (NUEVA: DRO + Ex-Ante walk-forward)
"""
import os
import sys
...
# ------------------------------------------------------------------
# 2. Pesos estáticos de muestra completa para Markowitz/HERC (ya
#    persistidos, se reusan tal cual para no reintroducir variacion
#    de punto flotante en paneles ya publicados) + DRO/Ex-Ante (nuevo
#    calculo puntual, UNA sola optimizacion sobre toda la muestra,
#    igual que se hizo originalmente para Markowitz/HERC/etc. en
#    pipe.main() paso 3).
# ------------------------------------------------------------------
...
w_markowitz = df_pesos_estaticos.loc["Markowitz", TICKERS].values.astype(float)
w_herc = df_pesos_estaticos.loc["HERC", TICKERS].values.astype(float)
...
pesos_estaticos_fig7 = {
    "Markowitz": w_markowitz,
    "HERC": w_herc,
    "DRO_Wasserstein": w_dro,
    "Min_CVaR_ExAnte": w_exante,
}

# ------------------------------------------------------------------
# 3. FIGURA 4 (regenerada): retorno acumulado + drawdown, 9 series
# ------------------------------------------------------------------
print("=== 3. Regenerando Figura 4 (retorno acumulado + drawdown, 9 modelos) ===")
...
# ------------------------------------------------------------------
# 4. FIGURA 6 (regenerada): CPCV boxplot, 8 estrategias
# ------------------------------------------------------------------
print("=== 4. Regenerando Figura 6 (CPCV, 8 estrategias) ===")
...
# ------------------------------------------------------------------
# 5. FIGURA 7 (regenerada): descomposicion de riesgo, 4 paneles
# ------------------------------------------------------------------
print("=== 5. Regenerando Figura 7 (descomposicion de riesgo, 4 paneles) ===")
...
generar_figura_7_extendida(pesos_estaticos_fig7, matriz_cov, TICKERS)
print("  fig07 regenerada.")

# ------------------------------------------------------------------
# 6. FIGURA 8 (nueva): evolucion de pesos walk-forward, DRO + Ex-Ante
# ------------------------------------------------------------------
print("=== 6. Generando Figura 8 (evolucion de pesos, DRO + Ex-Ante) ===")
...
def generar_figura_8_pesos_extension(df_dro, df_exante, tickers):
    fig, axes = plt.subplots(1, 2, figsize=(14, 5.2), sharex=True, sharey=True)
    paneles = [
        (df_dro, "A. DRO-Wasserstein ($\\varepsilon=0.02$, Robustez sin Concentración)", axes[0]),
        (df_exante, "B. Mínimo CVaR Ex-Ante (GARCH-EVT-Cópula $t$, Rotación Elevada)", axes[1]),
    ]
    paleta_activos = [
        "#1E3A8A", "#3B82F6", "#0D9488", "#10B981",
        "#F59E0B", "#D97706", "#64748B", "#8B5CF6", "#A8A29E"
    ]
    for df_w, titulo, ax in paneles:
        ax.stackplot(
            df_w.index,
        ...
        ax.set_title(titulo, pad=9, fontweight="bold", fontsize=11.0)
    ...
    fig.legend(
        handles, labels, loc="lower center", ncol=5, bbox_to_anchor=(0.5, -0.10),
        frameon=True, facecolor="#FFFFFF", edgecolor="#CBD5E1", fontsize=8.2
    )
    plt.tight_layout()
    plt.subplots_adjust(bottom=0.24)
    plt.savefig(os.path.join(FIGURAS, "fig08_evolucion_pesos_extension.png"), dpi=300, bbox_inches="tight")
    plt.close()


generar_figura_8_pesos_extension(df_w_dro, df_w_exante, TICKERS)
print("  fig08 generada.")
